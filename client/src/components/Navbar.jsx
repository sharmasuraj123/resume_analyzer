...
const Navbar = () => {
  ...
  return (
    <nav className="bg-[#0f1117] border-b border-gray-800 px-6 py-4 flex items-center justify-between sticky top-0 z-50">
      <Toaster position="top-right" />
      <Link
        to="/dashboard"
        className="flex items-center gap-2 text-blue-400 font-bold text-xl"
      >
        <FiFileText size={24} />
      ...
      {user && (
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={handleLogout}
            className="flex items-center gap-1.5 text-sm text-red-400 hover:text-red-300 font-medium transition"
          >
            <FiLogOut size={16} />
            <span className="hidden sm:inline">Logout</span>
          </button>
